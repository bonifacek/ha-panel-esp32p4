...
esp_err_t esp_lcd_touch_new_i2c_gsl3680(esp_lcd_panel_io_handle_t io, const esp_lcd_touch_config_t *config, esp_lcd_touch_handle_t *out_touch)
{
    ...
    // Wynik read_ram_fw nie trafia do ret — blad weryfikacji nie kasuje uchwytu.
    if (esp_lcd_touch_gsl3680_read_ram_fw(esp_lcd_touch_gsl3680) != ESP_OK) {
        ESP_LOGW(TAG, "Firmware verification failed (0xb0 != 0x5a), continuing");
    }
    // touch_gsl3680_read_cfg(esp_lcd_touch_gsl3680);
...
static esp_err_t esp_lcd_touch_gsl3680_init(esp_lcd_touch_handle_t tp)
{
    ...
    // Halt DSP (0xe0=0x88) i zatrzymaj CPU (0xe4=0x04) przed zaladowaniem firmware.
    // Wczesniejszy kod pisal do adresu 0x88 z wartoscia 0x01 (bledna zamiana rejestru
    // i danych) — DSP nigdy nie byl zatrzymywany, firmware nie startowal.
    buf[0] = 0x88;
    touch_gsl3680_i2c_write(tp, 0xe0, buf, 1);
    vTaskDelay(pdMS_TO_TICKS(5));
    buf[0] = 0x04;
    touch_gsl3680_i2c_write(tp, 0xe4, buf, 1);
    vTaskDelay(pdMS_TO_TICKS(5));
    buf[0] = 0x00;
    touch_gsl3680_i2c_write(tp, 0xe0, buf, 1);
    vTaskDelay(pdMS_TO_TICKS(20));

    // Zaladuj firmware do RAM chipa
    esp_lcd_touch_gsl3680_load_fw(tp);

    // Resetuj wewnetrzny licznik adresu, potem uruchom DSP 4 bajtami zer.
    // Wczesniejszy startup_chip pisal tylko 1 bajt — chip wymaga 4 bajtow do startu.
    memset(buf, 0, sizeof(buf));
    touch_gsl3680_i2c_write(tp, 0xbc, buf, 4);
    vTaskDelay(pdMS_TO_TICKS(5));
    touch_gsl3680_i2c_write(tp, 0xe0, buf, 4);
    vTaskDelay(pdMS_TO_TICKS(10));
