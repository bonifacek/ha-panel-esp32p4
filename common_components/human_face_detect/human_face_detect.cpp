...
MSR::MSR(const char *model_name)
{
    ...
    // Wejście: LE RGB565, poprawna kolejność kanałów (R w bitach [15:11]).
    // Brak bswap, brak RGB_SWAP — flagi = 0.
    ...
    m_postprocessor = new dl::detect::MSRPostprocessor(
        m_model, 0.3, 0.3, 10, {{8, 8, 9, 9, {{16, 16}, {32, 32}}}, {16, 16, 9, 9, {{64, 64}, {128, 128}}}});
}
...
#endif
#if CONFIG_IDF_TARGET_ESP32P4
    // LE RGB565, brak transformacji.
    m_image_preprocessor = new dl::image::ImagePreprocessor(
        m_model, {0, 0, 0}, {1, 1, 1}, 0);
#else
    m_image_preprocessor = new dl::image::ImagePreprocessor(m_model, {0, 0, 0}, {1, 1, 1}, DL_IMAGE_CAP_RGB_SWAP);
#endif
    m_postprocessor = new dl::detect::MNPPostprocessor(m_model, 0.03, 0.3, 10, {{1, 1, 0, 0, {{48, 48}}}});
}
