...
#endif
    dpi_config.num_fbs = CONFIG_BSP_LCD_DPI_BUFFER_NUMS;
    // DMA2D async copy silently fails for narrow strips (<~32px wide) — use synchronous CPU copy instead.
    // With sw_rotate=true the DSI flush path sends 6-px-wide physical strips; DMA2D's draw_sem
    // gets stuck at 0 after the first failed enqueue, silently dropping every subsequent tile
    // and leaving the framebuffer black.
